...
const CategoryFilterTest = () => {
  ...
  const testCategoryFiltering = async () => {
    ...
    try {
      ...
      // Test "All" posts
      const allPosts = await fetchBlogPosts(1, 5, 'All');
      categoryTests['All'] = {
      ...
      for (const category of categories.slice(0, 3)) { // Test first 3 categories
        try {
          const posts = await fetchBlogPosts(1, 5, category.name);
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-lg max-w-4xl mx-auto">
      <h2 className="text-2xl font-bold mb-4">Category Filtering Test</h2>
      
      <button
        onClick={testCategoryFiltering}
        disabled={loading}
        className="bg-[#3433fe] text-white px-6 py-2 rounded-md hover:bg-[#2d2bdb] disabled:opacity-50"
      >
        {loading ? 'Testing...' : 'Test Category Filtering'}
      </button>
              ...
              <div className="grid grid-cols-2 gap-2 mt-2">
                {testResults.categories.map((category, index) => (
                  <div key={index} className="text-sm">
                    <span className="font-mono">{category.id}</span> - {category.name}
                  </div>
                ))}
              </div>
            </div>
          ...
          {testResults.categoryTests && (
            <div className="p-4 bg-gray-100 rounded">
              ...
              {Object.entries(testResults.categoryTests).map(([categoryName, result]) => (
                <div key={categoryName} className="mt-2 p-2 bg-white rounded">
